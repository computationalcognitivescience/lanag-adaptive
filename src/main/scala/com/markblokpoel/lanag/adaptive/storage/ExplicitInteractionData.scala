...
/** Data storage for the entire interaction
  ...
  */
case class ExplicitInteractionData(initialInitiatorData: InitialInitiatorData,
                                   klInitItoR: BigNatural,
                                   klInitRtoI: BigNatural,
                                   initiatorData: List[ExplicitInitiatorData],
                                   responderData: List[ExplicitResponderData],
  ...
  def addInitiatorData(
      moreInitiatorData: ExplicitInitiatorData): ExplicitInteractionData =
    ExplicitInteractionData(initialInitiatorData,
                            klInitItoR,
                            klInitRtoI,
  ...
  def addResponderData(
      moreResponderData: ExplicitResponderData): ExplicitInteractionData =
    ExplicitInteractionData(initialInitiatorData,
                            klInitItoR,
                            klInitRtoI,
                            initiatorData,
  ...
  def addKLDivergence(
      ...
      responderToInitiator: BigNatural): ExplicitInteractionData =
    ExplicitInteractionData(
      ...
      initiatorData,
      responderData,
      klInitiatorToResponder :+ initiatorToResponder,
      klResponderToInitiator :+ responderToInitiator
    )
  override def toString: String = {
    s"==NEW ROUND==\n[Initiator] ${initialInitiatorData.intendedReferent} -> ${initialInitiatorData.signal}\n" +
      (for (i <- 0 until math.max(initiatorData.size, responderData.size))
        yield {
          val reData =
            if (responderData.isDefinedAt(i)) Some(responderData(i)) else None
          val inData =
            if (initiatorData.isDefinedAt(i)) Some(initiatorData(i)) else None
